...
export class Neo4jService implements OnModuleDestroy {
  ...
  async connect(uri: string, user: string, pass: string): Promise<boolean> {
    try {
      ...
      this.driver = null; // Trả về rỗng nếu lỗi
      throw new HttpException('Sai cấu hình hoặc không thể kết nối Neo4j', HttpStatus.UNAUTHORIZED);
    }
  }
  ...
  getReadSession(): Session {
    if (!this.driver) {
      throw new HttpException('Vui lòng kết nối Database trước!', HttpStatus.BAD_REQUEST);
    }
