...
def class_weights_from_mask(y, mask, num_classes=2):
    """
    =======================
    Sqrt inverse-frequency class weights from the train split only
    =======================
    Computed exclusively on nodes selected by `mask` to avoid
    leaking val/test distribution into the loss.

    Sqrt dampens the imbalance signal:
        weight[k] = sqrt(total / (C * count[k]))

    Effect at 1% fraud ratio: weight ~10x instead of linear ~99x.
    Recall decreases moderately; Precision increases substantially;
    F1-Macro improves overall.
    """
    y_train = y[mask]
    counts = torch.bincount(y_train, minlength=num_classes).float()
    counts = counts.clamp(min=1.0)
    linear_weights = counts.sum() / (num_classes * counts)
    return torch.sqrt(linear_weights)
